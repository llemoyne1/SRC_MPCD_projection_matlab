...
% Channel-safe Q9 options.  These are inactive by default to preserve the
% historical algebraic projection, but Poiseuille/channel runners can enable
% them to avoid feeding wall-layer population noise into the low-k mass-flux
% correction.
channelSafeMode = logical(get_param_default(params, 'massFluxChannelSafeMode', false));
wallGuardCells = get_param_default(params, 'massFluxWallGuardCells', 0);
normalGuardCells = get_param_default(params, 'massFluxNormalGuardCells', wallGuardCells);
suppressWallNormalNearWalls = logical(get_param_default(params, 'massFluxSuppressWallNormalNearWalls', channelSafeMode));
maxVelocityKick = get_param_default(params, 'massFluxMaxVelocityKick', Inf);
maxNormalVelocityKick = get_param_default(params, 'massFluxMaxNormalVelocityKick', Inf);
...
for k = 1:2:numel(varargin)
    ...
    switch key
        ...
        case "lowkmaxindex"
            lowKMaxIndex = val;
        case {"channelsafemode", "channelsafe"}
            channelSafeMode = logical(val);
        case {"wallguardcells", "guardcells"}
            wallGuardCells = val;
        case {"normalguardcells", "wallnormalguardcells"}
            normalGuardCells = val;
        case {"suppresswallnormalnearwalls", "suppressnormalnearwalls"}
            suppressWallNormalNearWalls = logical(val);
        case {"maxvelocitykick", "maxdv"}
            maxVelocityKick = val;
        case {"maxnormalvelocitykick", "maxduy"}
            maxNormalVelocityKick = val;
        otherwise
...
if channelSafeMode && wallGuardCells <= 0
    wallGuardCells = 2;
end
wallGuardCells = min(max(round(wallGuardCells), 0), floor(Ny/2));
normalGuardCells = min(max(round(normalGuardCells), 0), floor(Ny/2));
activeTargetMask = make_channel_bulk_mask(Nx, Ny, wallGuardCells);
...
switch modeLower
    ...
    case {"relax_to_uniform", "relax", "density_relax", "rho_relax"}
        ...
        targetRaw = apply_active_mask_zero_mean(targetRaw, activeTargetMask);
        targetRequested = apply_mass_flux_target_filter(targetRaw, Nx, Ny, targetFilter, lowKMaxIndex);
        targetRequested = apply_active_mask_zero_mean(targetRequested, activeTargetMask);
    ...
    case {"relax_to_uniform_lowk", "relax_lowk", "density_relax_lowk", "rho_relax_lowk"}
        targetRaw = (beta / dt) * (N(:) - gamma);
        targetRaw = apply_active_mask_zero_mean(targetRaw, activeTargetMask);
        if any(strcmpi(strrep(char(string(targetFilter)), '-', '_'), {'none','off','identity','raw'}))
...
if lowKCorrectionOnly
    ...
    rhsRequested = apply_mass_flux_target_filter(rhsFull, Nx, Ny, targetFilter, lowKMaxIndex);
    rhsRequested = apply_active_mask_zero_mean(rhsRequested, activeTargetMask);
...
correction = -D' * lambda;
mProjRaw = m + correction;
MxProjRaw = reshape(mProjRaw(1:Nc), [Nx, Ny]);
MyProjRaw = reshape(mProjRaw(Nc+1:end), [Nx, Ny]);
Nsafe = max(N, minCellCount);
UxProjRaw = MxProjRaw ./ Nsafe;
UyProjRaw = MyProjRaw ./ Nsafe;
UxProjRaw(N <= 0) = 0;
UyProjRaw(N <= 0) = 0;

dUx = UxProjRaw - Ux;
dUy = UyProjRaw - Uy;
[dUx, dUy, channelLimiterInfo] = apply_channel_safe_delta_filter(dUx, dUy, ...
    channelSafeMode, wallGuardCells, normalGuardCells, suppressWallNormalNearWalls, ...
    maxVelocityKick, maxNormalVelocityKick);
...
if lowKCorrectionOnly
    divBeforeDiagVec = apply_mass_flux_target_filter(divBeforeVec, Nx, Ny, targetFilter, lowKMaxIndex);
    divBeforeDiagVec = apply_active_mask_zero_mean(divBeforeDiagVec, activeTargetMask);
    divAfterDiagVec = apply_mass_flux_target_filter(divAfterVecFull, Nx, Ny, targetFilter, lowKMaxIndex);
    divAfterDiagVec = apply_active_mask_zero_mean(divAfterDiagVec, activeTargetMask);
...
proj.channelSafeMode = channelSafeMode;
proj.wallGuardCells = wallGuardCells;
proj.normalGuardCells = normalGuardCells;
proj.suppressWallNormalNearWalls = suppressWallNormalNearWalls;
proj.channelSafeActiveFraction = nnz(activeTargetMask) / numel(activeTargetMask);
proj.channelLimiter = channelLimiterInfo;
...
switch filterLower
    ...
    case {"lowpass_fft", "lowk", "fft_lowk"}
        ...
        target = real(ifft2(H));
        target = target(:);
    case {"lowpass_x", "lowk_x", "fft_x_lowk"}
        % Periodic filtering only along x.  This avoids imposing a periodic
        % spectral basis across bounded wall-normal rows.  The x-mean mode is
        % retained, so this variant can relax slow y-dependent density drifts.
        A = reshape(targetRaw, [Nx, Ny]);
        H = fft(A, [], 1);
        keepX = make_low_k_mask_1d(Nx, lowKMaxIndex, true);
        H(~keepX, :) = 0;
        target = real(ifft(H, [], 1));
        target = target(:);
    case {"lowpass_x_nonzero", "lowk_x_nonzero", "fft_x_lowk_nonzero"}
        % Safer first channel option: only x-dependent long waves are relaxed.
        % The x-mean wall-normal density profile is left to the physical wall
        % dynamics, avoiding artificial normal pumping at bounded y walls.
        A = reshape(targetRaw, [Nx, Ny]);
        H = fft(A, [], 1);
        keepX = make_low_k_mask_1d(Nx, lowKMaxIndex, false);
        H(~keepX, :) = 0;
        target = real(ifft(H, [], 1));
        target = target(:);
    otherwise
...
function keep = make_low_k_mask_1d(Nx, kmax, keepZeroMode)
ix = [0:floor(Nx/2), -ceil(Nx/2)+1:-1];
if numel(ix) > Nx
    ix = ix(1:Nx);
end
keep = abs(double(ix(:))) <= kmax;
if ~keepZeroMode
    keep(1) = false;
end
end

function mask = make_channel_bulk_mask(Nx, Ny, guardCells)
mask = true(Nx, Ny);
if guardCells > 0
    g = min(guardCells, floor(Ny/2));
    mask(:, 1:g) = false;
    mask(:, Ny-g+1:Ny) = false;
end
end

function vec = apply_active_mask_zero_mean(vec, activeMask)
A = reshape(double(vec), size(activeMask));
A(~activeMask) = 0;
if any(activeMask(:))
    A(activeMask) = A(activeMask) - mean(A(activeMask));
end
A(~activeMask) = 0;
vec = A(:);
end

function [dUx, dUy, info] = apply_channel_safe_delta_filter(dUx, dUy, channelSafeMode, wallGuardCells, normalGuardCells, suppressWallNormalNearWalls, maxVelocityKick, maxNormalVelocityKick)
info = struct();
info.enabled = logical(channelSafeMode);
info.wallGuardCells = wallGuardCells;
info.normalGuardCells = normalGuardCells;
info.suppressWallNormalNearWalls = logical(suppressWallNormalNearWalls);
info.maxVelocityKick = maxVelocityKick;
info.maxNormalVelocityKick = maxNormalVelocityKick;
info.nZeroedTangentialCells = 0;
info.nZeroedNormalCells = 0;
info.nVectorLimitedCells = 0;
info.nNormalLimitedCells = 0;
info.rmsBefore = sqrt(mean(dUx(:).^2 + dUy(:).^2));
info.maxAbsDUxBefore = max(abs(dUx(:)));
info.maxAbsDUyBefore = max(abs(dUy(:)));

[Nx, Ny] = size(dUx); %#ok<ASGLU>
if channelSafeMode && wallGuardCells > 0
    g = min(wallGuardCells, floor(Ny/2));
    rows = [1:g, Ny-g+1:Ny];
    dUx(:, rows) = 0;
    dUy(:, rows) = 0;
    info.nZeroedTangentialCells = numel(dUx(:, rows));
    info.nZeroedNormalCells = numel(dUy(:, rows));
end

if channelSafeMode && suppressWallNormalNearWalls && normalGuardCells > 0
    g = min(normalGuardCells, floor(Ny/2));
    rows = [1:g, Ny-g+1:Ny];
    dUy(:, rows) = 0;
    info.nZeroedNormalCells = max(info.nZeroedNormalCells, numel(dUy(:, rows)));
end

if isfinite(maxNormalVelocityKick) && maxNormalVelocityKick >= 0
    before = dUy;
    dUy = max(min(dUy, maxNormalVelocityKick), -maxNormalVelocityKick);
    info.nNormalLimitedCells = nnz(dUy ~= before);
end

if isfinite(maxVelocityKick) && maxVelocityKick > 0
    mag = sqrt(dUx.^2 + dUy.^2);
    limited = mag > maxVelocityKick;
    scale = ones(size(mag));
    scale(limited) = maxVelocityKick ./ max(mag(limited), eps);
    dUx = dUx .* scale;
    dUy = dUy .* scale;
    info.nVectorLimitedCells = nnz(limited);
end

info.rmsAfter = sqrt(mean(dUx(:).^2 + dUy(:).^2));
info.maxAbsDUxAfter = max(abs(dUx(:)));
info.maxAbsDUyAfter = max(abs(dUy(:)));
end
