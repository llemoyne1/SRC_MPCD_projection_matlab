...
sgtitle(sprintf(['%s step %d, t=%.4g %s | Nact=%d/%d free=%d inserted=%s ', ...
                 '| N[min,max]=[%g,%g] MrelRMS=%.2e mRelStd=%.2e kBT=%.3g'], ...
    char(string(method)), step, step * getf(params,'dt',0), opts.titleSuffix, ...
    md.NpActive, md.Ncapacity, md.Nfree, format_scalar(getf(opts.insertDiag,'nInsertedParticles',NaN)), ...
...
subplot(2,3,6);
imagesc(xc, yc, classMap'); axis xy equal tight; colorbar;
