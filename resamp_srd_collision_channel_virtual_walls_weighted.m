function [vOut, info] = resamp_srd_collision_channel_virtual_walls_weighted(x, v, m, params)
%RESAMP_SRD_COLLISION_CHANNEL_VIRTUAL_WALLS_WEIGHTED Weighted SRD collision in a channel.
%
%   [vOut, info] = resamp_srd_collision_channel_virtual_walls_weighted(x,v,m,params)
%
% Weighted counterpart of mpcd_srd_collision_channel_virtual_walls.  Real
% particles have arbitrary masses m.  Virtual wall particles are not stored
% in the state; they contribute only aggregate mass and momentum to the SRD
% collision-cell mean.  Only real fluid particles are updated.
%
% The default wall VP geometry is 'shifted_solid_fraction': a randomly
% shifted collision grid is allowed to intersect y<0 and y>Ly.  The solid
% fraction of those shifted cells receives virtual wall mass proportional to
% gamma*densityFactor.  This restores near-wall collision statistics and
% improves no-slip behavior without adding transported particles to the pool.

validate_state_arrays(x, v, m);
...
useRandomGridShiftX = logical(get_param(params, 'useRandomGridShiftX', get_param(params, 'useRandomGridShift', true)));
...
switch geometryMode
    case 'shifted_solid_fraction'
        [iyExt, Ncy, iyRaw] = assign_y_cells_extended(x(:,2), shiftY, dy, Ny);
        cellId = iyExt + Ncy * (ix - 1);
        Nc = Nx * Ncy;
        [Mvirt, PxVirt, PyVirt, NvirtEff, virtInfo] = virtual_wall_moments_shifted_solid_fraction(params, Nx, Ny, Ncy, Nc, dy, Ly, shiftY);
    case {'fixed_boundary_layers','fixed_layers','boundary_layers'}
        ...
        cellId = iy + Ny * (ix - 1);
        Nc = Nx * Ny;
        [Mvirt, PxVirt, PyVirt, NvirtEff, virtInfo] = virtual_wall_moments_fixed_layers(params, Nx, Ny, Nc);
    otherwise
        if enabled
...
Mreal = accumarray(cellId, m, [Nc, 1], @sum, 0);
PxReal = accumarray(cellId, m .* v(:, 1), [Nc, 1], @sum, 0);
PyReal = accumarray(cellId, m .* v(:, 2), [Nc, 1], @sum, 0);
...
Ux = zeros(Nc, 1);
Uy = zeros(Nc, 1);
occTot = Mtot > eps(max(1.0, mean(Mtot(Mtot>0), 'omitnan')));
Ux(occTot) = PxTot(occTot) ./ Mtot(occTot);
Uy(occTot) = PyTot(occTot) ./ Mtot(occTot);
...
PbeforeReal = [sum(m .* v(:,1), 'omitnan'), sum(m .* v(:,2), 'omitnan')];
PafterReal  = [sum(m .* vOut(:,1), 'omitnan'), sum(m .* vOut(:,2), 'omitnan')];
...
info.McellReal = Mreal;
info.NcellVirtualEff = NvirtEff;
info.McellVirtual = Mvirt;
info.McellTotal = Mtot;
info.PxReal = PxReal;
info.PyReal = PyReal;
...
info.MMeanReal = mean(double(Mreal), 'omitnan');
info.MStdReal = std(double(Mreal), 0, 'omitnan');
info.nEmptyRealCells = nnz(Nreal == 0);
info.MMeanTotalCollision = mean(double(Mtot), 'omitnan');
info.MStdTotalCollision = std(double(Mtot), 0, 'omitnan');
info.nVirtualCells = nnz(Mvirt > 0);
info.virtualMassTotal = sum(Mvirt, 'omitnan');
info.nVirtualParticlesEquivalentTotal = sum(NvirtEff, 'omitnan');
info.virtualMomentumXTotal = sum(PxVirt, 'omitnan');
info.virtualMomentumYTotal = sum(PyVirt, 'omitnan');
info.realMomentumBefore = PbeforeReal;
info.realMomentumAfter = PafterReal;
info.realMomentumDelta = PafterReal - PbeforeReal;
info.realMomentumDeltaNorm = norm(info.realMomentumDelta);
end
...
% maps it to MATLAB indices 1..Ny+2.
...
function [Mvirt, PxVirt, PyVirt, NvirtEff, info] = virtual_wall_moments_shifted_solid_fraction(params, Nx, Ny, Ncy, Nc, dy, Ly, shiftY)
Mvirt = zeros(Nc, 1);
PxVirt = zeros(Nc, 1);
PyVirt = zeros(Nc, 1);
NvirtEff = zeros(Nc, 1);
...
virtualMassPerParticle = get_param(params, 'wallVirtualParticleMass', get_param(params, 'resampParticleMass', 1.0));
virtualMassPerParticle = max(realmin, double(virtualMassPerParticle));
...
info.virtualMassPerParticle = virtualMassPerParticle;
...
for ix = 1:Nx
    for jyRaw = 0:(Ny+1)
        ...
        if includeBottom && bottomFrac > 0
            ...
            [Mvirt, PxVirt, PyVirt, NvirtEff] = add_virtual_to_cell(Mvirt, PxVirt, PyVirt, NvirtEff, id, nAdd, virtualMassPerParticle, Ubottom, Vbottom, kBT, thermal);
        ...
        if includeTop && topFrac > 0
            ...
            [Mvirt, PxVirt, PyVirt, NvirtEff] = add_virtual_to_cell(Mvirt, PxVirt, PyVirt, NvirtEff, id, nAdd, virtualMassPerParticle, Utop, Vtop, kBT, thermal);
...
info.nVirtualTotal = sum(NvirtEff, 'omitnan');
info.virtualMassTotal = sum(Mvirt, 'omitnan');
info.PxVirtualTotal = sum(PxVirt, 'omitnan');
info.PyVirtualTotal = sum(PyVirt, 'omitnan');
end

function [Mvirt, PxVirt, PyVirt, NvirtEff, info] = virtual_wall_moments_fixed_layers(params, Nx, Ny, Nc)
Mvirt = zeros(Nc, 1);
PxVirt = zeros(Nc, 1);
PyVirt = zeros(Nc, 1);
NvirtEff = zeros(Nc, 1);
...
if includeBottom
    ...
    for k = 1:numel(ids)
        [Mvirt, PxVirt, PyVirt, NvirtEff] = add_virtual_to_cell(Mvirt, PxVirt, PyVirt, NvirtEff, ids(k), nPerCell, virtualMassPerParticle, Ubottom, Vbottom, kBT, thermal);
    end
...
if includeTop
    ...
    for k = 1:numel(ids)
        [Mvirt, PxVirt, PyVirt, NvirtEff] = add_virtual_to_cell(Mvirt, PxVirt, PyVirt, NvirtEff, ids(k), nPerCell, virtualMassPerParticle, Utop, Vtop, kBT, thermal);
    end
...
function [Mvirt, PxVirt, PyVirt, NvirtEff] = add_virtual_to_cell(Mvirt, PxVirt, PyVirt, NvirtEff, id, nAdd, mVirt, Uwall, Vwall, kBT, thermal)
if nAdd <= 0
    return;
end
Madd = nAdd * mVirt;
Mvirt(id) = Mvirt(id) + Madd;
NvirtEff(id) = NvirtEff(id) + nAdd;
if thermal
    sigmaP = sqrt(max(Madd * kBT, 0));
    PxAdd = Madd * Uwall + sigmaP * randn();
    PyAdd = Madd * Vwall + sigmaP * randn();
else
    PxAdd = Madd * Uwall;
    PyAdd = Madd * Vwall;
end
...
info.virtualMassPerParticle = NaN;
...
info.virtualMassTotal = 0.0;
info.PxVirtualTotal = 0.0;
info.PyVirtualTotal = 0.0;
end

function validate_state_arrays(x, v, m)
if size(x, 2) ~= 2 || size(v, 2) ~= 2 || size(x, 1) ~= size(v, 1) || size(x, 1) ~= numel(m)
    error('x, v and m must have matching particle count; x and v must be Np-by-2.');
end
m = m(:);
